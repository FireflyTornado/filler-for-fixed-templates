...
4. 在下方变量表点击目标变量，上方“当前变量”只显示所选名称和类型。再选择来源单元格和定位方式。默认“全部同类映射”使用当前工作表上方的全局取值行或列；选择“仅此映射”后才会显示该映射自己的行或列选择框，点击数据格会同步更新它。点击“添加／更新映射”。
...
“按行列标题”在创建映射时保存所选格的行标题和列标题，切换工作表后使用该表自己的标题行、标题列重新寻找交叉格。“锁定”模式同样认标题；选定行必须在表头下方，选定列必须在行标题列右侧。固定单元格、按行列标题模式不显示单条取值位置。切换选择只更新预览，应用后才修改模板变量。

每张工作表分别保存标题行、标题列、全局取值行和全局取值列。“全部同类映射”的行选择只影响当前表的锁定列映射，列选择只影响当前表的锁定行映射；“仅此映射”保留该条自己的行或列。旧映射默认按“全部同类映射”读取。上方 Excel 数据区只高亮当前单元格；在单条模式点击数据格会同步其行或列，点击行号或列号会调整当前范围对应的位置。
