...
3. 选择工作表，设置表头行、行标题列（A=1）和当前记录行。点选单元格或输入 `D8` 等地址，查看标题及内容。
4. 选择定位方式和目标变量，点击“添加／更新映射”。一个变量对应一条映射，同一来源可以映射给多个变量。
5. 对比显示内容、实际填入值、原值和状态；处理错误后点击“应用到模板变量”。
...
| 当前记录／列标题 | 每行是一条客户、项目或业务记录 | 复用列映射，选择不同记录行预览其数据 |

“同名绑定建议…”可根据唯一同名列提出映射，由用户确认。已有映射可以停用、更新或改为手工填写；不再存在的变量映射可用“清理失效映射”删除。
...
- **空值**：默认报错；每条映射可选择保留原值，或允许空文本清除。缺失、错误或有歧义的来源不会自动变成 0。
...
| Excel 映射无法恢复 | 检查工作表、表头行、标题唯一性和定位方式，必要时重新绑定 |
