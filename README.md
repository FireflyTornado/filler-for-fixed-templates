...
3. 选择工作表，设置“列标题所在行”（如第 1 行）和“行标题所在列”（如 A 列）。标题位置及结构保存在模板映射中；“当前表统一取值行／列”只属于当前打开的 Excel，各工作表分别记忆，换一个文件便重新取该表标题之后的首行、首列。预览顶部将列号和列标题分成两行，左侧将行号和行标题分成两列；滚动时标题区保持可见，长标题可悬停查看全文。
...
每张逻辑工作表只保存标题骨架、标题行和标题列。匹配的新表整体移动时，程序会换算标题位置和“仅此映射”的单独位置；“全部同类映射”始终读取当前 Excel 中该实际工作表的统一取值行或列，不继承旧文件坐标。刷新同一个文件会保留本次选择，打开另一个文件会重置。多个工作表结构相同时会报告歧义，不会擅自取值。旧映射默认按“全部同类映射”读取，旧配置里保存的全局行列会在读取时忽略。
