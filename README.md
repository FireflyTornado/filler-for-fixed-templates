...
4. 在下方变量表点击目标变量，上方“当前变量”只显示所选名称和类型。再选择来源单元格和定位方式。锁定列时选择第几行，锁定行时选择第几列，并设置“选择范围”：默认“全部同类映射”会让同方向映射跟随全局行或列；“仅此映射”会把行或列单独保存在当前映射中。点击“添加／更新映射”。
...
“全部同类映射”的行选择只影响锁定列映射，列选择只影响锁定行映射；两者互不覆盖。“仅此映射”保留该条自己的行或列。旧映射默认按“全部同类映射”读取。上方 Excel 数据区只高亮当前单元格，点击行号或列号仅改变相应的选定行列。
