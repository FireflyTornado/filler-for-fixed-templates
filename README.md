...
| 变量与计算 | 支持数值、短字符串、多行文本、算术表达式和内置日期变量 |
...
更换 Excel 或恢复映射只更新预览，必须点击“应用到模板变量”才会覆盖变量。修改模板、变量或日期后，旧结果会失效，需要重新生成。
...
3. 选择工作表，设置“列标题所在行”（如第 1 行）和“行标题所在列”（如 A 列，可输入字母或列序号）。预览顶部将列号和列标题分成两行，左侧将行号和行标题分成两列；滚动时标题区保持可见，长标题可悬停查看全文。
4. 在下方变量表点击目标变量，上方“当前变量”只显示所选名称和类型。再选择来源单元格和定位方式。锁定列时，在定位方式旁的“选定行列”选择第几行；锁定行时选择第几列（同时显示列字母）。点击“添加／更新映射”。一个变量对应一条映射，同一来源可以映射给多个变量。
5. 查看映射表下方的实时说明：实际来源行列、来源内容、目标模板及变量、原值和预计填入值。待保存的映射需先添加／更新；处理错误后点击“应用到模板变量”。
...
“锁定”认标题，标题位置移动后仍会尝试重新定位。选定行必须在表头下方，选定列必须在行标题列右侧；行和列分别记忆，仅影响各自的锁定模式。固定单元格、按行列标题模式不显示“选定行列”。切换选择只更新预览，应用后才修改模板变量。
...
昨日、明日也支持“年／年月／年月日”形式，上月、本月、下月支持“年／年月”形式。完整列表和运算示例可在 F1 帮助中查看。
