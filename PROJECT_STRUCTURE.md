...
`MappingProfile.Binding` 保存目标变量、工作表提示、定位方式、0 起始坐标、列标题所在行、行标题所在列、标题上下文和空值策略。不存在启用状态字段；Excel 文件路径及文件名不参与映射身份。

| 定位方式 | 匹配规则 |
| --- | --- |
| `FIXED` | 使用坐标，并检查已保存的结构锚点；无锚点时要求在本次文件上显式绑定 |
| `TITLES` | 查找包含表头上下文的标题行，再唯一定位目标列和行标题；允许行列移动 |
| `RECORD` | 锁定列／更改选定行：按列标题匹配，读取活动工作表的选定行 |
| `COLUMN_RECORD` | 锁定行／更改选定列：按行标题及纵向标题上下文匹配，读取活动工作表的选定列；不要求每个数据列有表头 |

`Binding.headers` 在 `COLUMN_RECORD` 中保存行标题列的纵向标题列表，在其他模式中保存表头行的横向标题列表。行、列选择分别传入解析器，互不覆盖。列选择必须位于匹配到的行标题列右侧；合并标题跨多行或标题重复时拒绝定位。

`DataExtractionPanel` 在后台同时生成已保存映射的整批预览和当前编辑映射的单条预览。下方说明使用解析后的 `Match`，展示实际行列、模板、变量、原值和填入值；有未保存的映射编辑时禁止应用旧映射。空值或格式错误也保留已定位的来源，避免说明退回旧坐标。

`SpreadsheetPreview` 将原始数据表、冻结的行号/行标题表以及列号/列标题区分开，滚动位置由 `JScrollPane` 同步。数据表坐标直接对应 Excel 的 0 起始行列，不再包含偏移一列的行号数据。数据表唯一管理共享列宽，避免标题行布局改变数据区的列宽。变量表是唯一变量选择入口，预览刷新后保留所选行。

同名工作表结构匹配时优先使用；否则可查找其他工作表。多个候选、必要标题缺失或合并标题造成歧义时不猜测。未映射的变量保留手工输入；已删除变量的映射保留但不执行，直到用户清理。
...
模板配置通过模板完整相对路径关联。当前配置格式版本为 4，映射子格式版本为 3；缺少映射字段的旧配置按空映射读取。旧 `RECORD` 身份不变，旧 `ERROR`、`CLEAR` 空值策略读取为 `KEEP`。`TemplateConfigStore.migrateLegacyMappingStates` 在启动时扫描并原子改写旧配置：删除 `enabled:false` 的映射，移除其余 `enabled` 字段，保留变量和其他原始字段；单文件加载也执行检测。运行时模型和匹配器不保留停用分支。配置模型示例：
...
```json
{
  ...
  "decimalPlaces": 2,
  "variables": {},
  "dataExtraction": { "version": 3, "bindings": [] }
}
```
