...
“模板填充”和“数据提取”共享一个 `TemplateSession`。恢复映射、切换工作表或当前记录、刷新 Excel 都只更新预览；显式应用才修改变量。生成操作及其快捷键只在“模板填充”页生效。
...
`MappingProfile.Binding` 保存目标变量、工作表提示、定位方式、0 起始坐标、表头行、行标题列、标题上下文、空值策略和启用状态。Excel 文件路径及文件名不参与映射身份。

| 定位方式 | 匹配规则 |
| --- | --- |
| `FIXED` | 使用坐标，并检查已保存的结构锚点；无锚点时要求在本次文件上显式绑定 |
| `TITLES` | 查找包含表头上下文的标题行，再唯一定位目标列和行标题；允许行列移动 |
| `RECORD` | 按列标题匹配，数据行使用对应活动工作表的当前记录 |

同名工作表结构匹配时优先使用；否则可查找其他工作表。多个候选、必要标题缺失或合并标题造成歧义时不猜测。未映射或停用的变量保留手工输入；已删除变量的映射保留但不执行，直到用户清理。

文本导入使用显示值，数值导入使用底层数值。空值按映射策略处理，不能沿用手工数值留空按 0 的规则。应用前验证整批数据，避免部分变量先被覆盖。
...
模板配置通过模板完整相对路径关联。当前配置格式版本为 4，映射子格式版本为 1；缺少映射字段的旧配置按空映射读取。配置模型示例：
...
```json
{
  ...
  "decimalPlaces": 2,
  "variables": {},
  "dataExtraction": { "version": 1, "bindings": [] }
}
```
