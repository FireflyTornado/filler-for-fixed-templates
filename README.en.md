...
**Use a Word template**: in Word, write the parts to be filled as `{{variable}}` (numbers / expressions / date variables) or `[[string]]` (multi-line text), save as `.docx`, put it into `Templates/`, then select it with "Choose Template File…". The template is shown as a read-only preview and "New Template / Save Template" are disabled automatically; fill in the inputs, click "Generate Result" to get the filled Word document, then "Save Result to File" to export it (default `result.docx`).
