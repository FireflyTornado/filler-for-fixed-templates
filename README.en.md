...
3. Select a worksheet and set its Column Titles Row (`列标题所在行`) and Row Titles Column (`行标题所在列`). These title positions and the title structure are stored in the template mapping. Current Sheet Shared Row/Column (`当前表统一取值行／列`) belongs only to the open Excel workbook, is remembered separately for each sheet, and resets to the first row/column after the titles when another file is opened. Column letters/titles and row numbers/titles remain separately visible while scrolling.
4. Click a variable in the lower table; Current Variable (`当前变量`) displays its name and type. Select the source cell and positioning mode. All Similar Mappings (`全部同类映射`) uses the source worksheet's global row or column. This Mapping Only (`仅此映射`) reveals an independent row or column selector, which follows the cell clicked in the source table. Then select `添加／更新映射`.
...
Each saved mapping follows its logical source worksheet by title structure. A matching name is preferred only when its structure still matches; a uniquely matching renamed worksheet is accepted automatically. Stored header positions and local row or column selections move with the matched table. Global selections always come from the matching physical sheet in the currently open workbook; refreshing that file retains them, while opening another file resets them. Ambiguous duplicate structures require the user to choose rather than silently reading the wrong sheet. Switching the worksheet shown in the editor does not redirect other mappings, so Apply All Variables can read several worksheets in one batch.
